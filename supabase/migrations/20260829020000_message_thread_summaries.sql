-- Scalable inbox summary for authenticated users.
-- Returns only conversations involving auth.uid(), ordered by latest activity,
-- with the last message and unread count computed in Postgres.
...
revoke all on function public.get_message_threads() from anon;
