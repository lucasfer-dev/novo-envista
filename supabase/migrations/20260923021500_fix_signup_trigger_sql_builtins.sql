-- Fix signup trigger failures caused by schema-qualifying SQL special forms
-- (COALESCE/NULLIF) and by an over-escaped birth-date regex.
...
begin
  ...
  if nullif(pg_catalog.btrim(coalesce(raw_cpf, '')), '') is not null then
    ...
    if exists (
      select 1 from public.account_private_identifiers
