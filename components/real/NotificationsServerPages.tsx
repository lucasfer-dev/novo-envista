...
export async function NotificationsServerPage({expectedRole,searchParams}:{expectedRole:ProductRole;searchParams:Search}){
 ...
 return <ProductShell user={appUser} title="Notificações" variant="legacyDark"><NotificationsView notifications={(data??[]) as never[]} status={first(query.status)}/></ProductShell>;
}
