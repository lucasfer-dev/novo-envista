...
export async function LearnServerPage(){
 ...
 return <LegacySocialShell user={appUser} role="participant" pathname="/app/learn"><LearnView courses={cards as never[]}/></LegacySocialShell>;
}
...
export async function CourseServerPage({slug,searchParams}:{slug:string;searchParams:Search}){
 ...
 return <LegacySocialShell user={appUser} role="participant" pathname={`/app/learn/${slug}`}><CourseView course={course as never} modules={modules as never[]} enrolled={Boolean(enrollment)} status={first(query.status)} error={first(query.error)}/></LegacySocialShell>;
}
