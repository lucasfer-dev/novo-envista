...
export async function LearnServerPage(){
 ...
 return <ProductShell user={appUser} title="Aprender"><LearnView courses={cards as never[]}/></ProductShell>;
}
...
export async function CourseServerPage({slug,searchParams}:{slug:string;searchParams:Search}){
 ...
 return <ProductShell user={appUser} title={course.title}><CourseView course={course as never} modules={modules as never[]} enrolled={Boolean(enrollment)} status={first(query.status)} error={first(query.error)}/></ProductShell>;
}
...
export async function LessonServerPage({slug,lessonId,searchParams}:{slug:string;lessonId:string;searchParams:Search}){
 ...
 return <ProductShell user={appUser} title={lesson.title}><LessonView course={course as never} lesson={lesson as never} modules={modules as never[]} completed={Boolean(lesson.completed)} status={first(query.status)} error={first(query.error)}/></ProductShell>;
}
