 ...
 return <>
  ...
  <div className={styles.grid}>{courses.length===0?<div className={styles.empty}>Nenhum curso publicado no momento.</div>:courses.map(course=>{
    ...
    return <article className={styles.card} key={course.id}>
      ...
      <Link prefetch={false} className={styles.primary} href={`/app/learn/${course.slug}`}>{course.enrolled?"Continuar curso":"Ver curso"}</Link>
    </article>;
  })}</div>
 </>;
}
...
export function CourseView({course,modules,enrolled,status,error}:{course:{id:string;slug:string;title:string;description:string;instructor:string;level:string;duration_minutes:number};modules:Module[];enrolled:boolean;status?:string;error?:string}){
 ...
 return <>
  ...
  <div className={styles.modules}>{modules.map(module=><section className={styles.module} key={module.id}><h2>{module.position}. {module.title}</h2>{module.lessons.map(lesson=><div className={styles.lesson} key={lesson.id}><div><Link prefetch={false} href={enrolled?`/app/learn/${course.slug}/lesson/${lesson.id}`:`/app/learn/${course.slug}`}>{lesson.completed?"✓ ":""}{lesson.title}</Link><p className={styles.muted}>{lesson.description}</p></div>{lesson.completed?<span className={styles.complete}>Concluída</span>:null}</div>)}</section>)}</div>
 </>;
}
...
export function LessonView({course,lesson,modules,completed,status,error}:{course:{slug:string;title:string};lesson:{id:string;title:string;description:string;content_md:string};modules:Module[];completed:boolean;status?:string;error?:string}){
 return <>
  ...
  <div className={styles.lessonPage}>
    ...
    <aside className={styles.sidebar}><h3>Conteúdo do curso</h3>{modules.map(module=><div key={module.id}><strong>{module.title}</strong>{module.lessons.map(item=><p key={item.id}><Link prefetch={false} href={`/app/learn/${course.slug}/lesson/${item.id}`}>{item.completed?"✓ ":""}{item.title}</Link></p>)}</div>)}</aside>
  </div>
 </>;
}
