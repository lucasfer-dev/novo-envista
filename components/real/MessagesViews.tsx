...
export function ConversationView({ role, currentUserId, conversationId, target, messages, blockedByMe, blockedMe, canSend, historyMode = false, hasOlder = false, olderCursor, status, error }: { role: Role; currentUserId: string; conversationId: string; target: { id: string; display_name: string; username: string | null }; messages: Message[]; blockedByMe: boolean; blockedMe: boolean; canSend: boolean; historyMode?: boolean; hasOlder?: boolean; olderCursor?: string | null; status?: string; error?: string }) {
  ...
  return <>
    <div className={styles.head}>
      ...
      <div className={styles.actions}>{blockedByMe ? <form action={unblockUserAction}><input type="hidden" name="blocked_id" value={target.id} /><button className={styles.secondary}>Desbloquear</button></form> : <form action={blockUserAction}><input type="hidden" name="blocked_id" value={target.id} /><button className={styles.danger}>Bloquear</button></form>}</div>
    </div>
    ...
    <section className={styles.card}>
      <div className={styles.actions} style={{ justifyContent: "space-between", marginBottom: 12 }}>
        {hasOlder && olderHref ? <Link className={styles.secondary} href={olderHref}>← Mensagens anteriores</Link> : <span />}
        {historyMode ? <Link className={styles.secondary} href={returnTo}>Voltar às mais recentes</Link> : null}
      </div>
