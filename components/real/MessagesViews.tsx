...
type Thread = { id:string; targetId:string; targetName:string; targetUsername:string | null; lastBody:string | null; lastAt:string | null };
...
export function MessagesIndexView({role,threads,status,error}:{role:Role;threads:Thread[];status?:string;error?:string}){
 ...
 const errors:Record<string,string>={user:"Informe um @usuário.",unavailable:"Essa conta não está disponível para novas mensagens.",blocked:"A conversa não pode ser iniciada por causa de um bloqueio.",create:"Não foi possível criar a conversa."};
 return <>
  <div className={styles.head}><div><h1>Mensagens</h1><p className={styles.muted}>Conversas privadas persistidas no Supabase.</p></div></div>
  ...
  <div className={styles.grid}>
   ...
   <section className={styles.panel}>
    ...
    <div className={styles.stack}>
     {threads.length===0?<div className={styles.empty}>Você ainda não tem conversas reais.</div>:threads.map((thread)=><Link className={styles.thread} href={`${root}/${thread.id}`} key={thread.id}>
...
export function ConversationView({role,currentUserId,conversationId,target,messages,blockedByMe,blockedMe,canSend,status,error}:{role:Role;currentUserId:string;conversationId:string;target:{id:string;display_name:string;username:string|null};messages:Message[];blockedByMe:boolean;blockedMe:boolean;canSend:boolean;status?:string;error?:string}){
 const root=base(role); const returnTo=`${root}/${conversationId}`;
 return <>
