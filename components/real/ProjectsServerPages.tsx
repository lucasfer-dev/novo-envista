...
import ProjectPortfolioPanel from "@/components/real/ProjectPortfolioPanel";
...
const projectFields = "id,slug,title,short_description,problem,solution,impact,needs,website_url,repository_url,stage,category,location,tags,readme,visibility,owner_user_id,owner_team_id,created_by,owner_team:teams!projects_owner_team_id_fkey(name,slug),owner_user:profiles!projects_owner_user_id_fkey(display_name,username)";
...
export async function ProjectServerDetail({expectedRole,slug,searchParams}:{expectedRole:ProductRole;slug:string;searchParams:Search}){
 ...
 const {data:updates}=await supabase.from("posts").select("id,body,created_at").eq("project_id",project.id).eq("visibility","platform").order("created_at",{ascending:false}).limit(10);
 const returnTo=`${expectedRole==="investor"?"/investor":"/app"}/projects/${project.slug}`;
 return <ProductShell user={appUser} title={project.title}>{expectedRole==="investor"&&project.visibility==="platform"?<><div style={{display:"flex",justifyContent:"flex-end",marginBottom:12}}><SaveProjectButton projectId={project.id} returnTo={returnTo}/></div><ProjectInterestPanel projectId={project.id} returnTo={returnTo}/></>:null}<ProjectPortfolioPanel project={project} updates={(updates??[]) as never[]} canEdit={canEdit}/><ProjectDetailView role={expectedRole} project={project} canEdit={canEdit} canDelete={canDelete} status={first(query.status)} error={first(query.error)}/></ProductShell>;
}
