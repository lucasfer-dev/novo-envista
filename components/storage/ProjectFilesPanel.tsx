...
export default async function ProjectFilesPanel({ projectId, slug, canEdit }: { projectId: string; slug: string; canEdit: boolean }) {
  ...
  const files = await Promise.all((attachments ?? []).map(async (item) => {
    const { data } = await supabase.storage.from("project-assets").createSignedUrl(item.path, 900);
  ...
  return (
    <div className={styles.box}>
      ...
      <p className={styles.hint}>Bucket privado. Links de download expiram em 15 minutos.</p>
      ...
      <div className={styles.files}>
        {files.length === 0 ? <span className={styles.hint}>Nenhum arquivo enviado.</span> : files.map((file) => (
          <div className={styles.file} key={file.id}>
            <div>{file.url ? <a href={file.url} target="_blank" rel="noreferrer">{file.file_name}</a> : <strong>{file.file_name}</strong>}<small>{Math.ceil(file.size_bytes / 1024)} KB · {file.mime_type}</small></div>
