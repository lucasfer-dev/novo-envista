...
export async function enrollCourseAction(formData:FormData){
  ...
  const courseId=text(formData,"course_id",80),slug=text(formData,"slug",180);
  ...
  revalidatePath(`/app/learn/${slug}`);redirect(`/app/learn/${slug}?status=enrolled`);
}

export async function completeLessonAction(formData:FormData){
  const {supabase,userId}=await requireProductUser("participant");
  const lessonId=text(formData,"lesson_id",80),slug=text(formData,"slug",180);
  if(!lessonId||!slug)redirect("/app/learn");
  const {error}=await supabase.from("lesson_progress").insert({lesson_id:lessonId,user_id:userId});
  if(error&&error.code!=="23505")redirect(`/app/learn/${slug}/lesson/${lessonId}?error=progress`);
  revalidatePath(`/app/learn/${slug}`);revalidatePath(`/app/learn/${slug}/lesson/${lessonId}`);redirect(`/app/learn/${slug}/lesson/${lessonId}?status=completed`);
}

export async function undoLessonAction(formData:FormData){
  const {supabase,userId}=await requireProductUser("participant");
  const lessonId=text(formData,"lesson_id",80),slug=text(formData,"slug",180);
  if(lessonId)await supabase.from("lesson_progress").delete().eq("lesson_id",lessonId).eq("user_id",userId);
  revalidatePath(`/app/learn/${slug}`);redirect(`/app/learn/${slug}/lesson/${lessonId}`);
}
