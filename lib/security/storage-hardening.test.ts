...
describe("storage hardening", () => {
  ...
  it("preflights the same quotas in the uploader for useful feedback", () => {
    ...
    const normalized = compact(uploader);
    expect(normalized).toContain("MAX_PROJECT_FILES=50");
    expect(normalized).toContain("MAX_PROJECT_BYTES=100*1024*1024");
    expect(normalized).toContain('from("project_attachments").select("size_bytes")');
  });
});
