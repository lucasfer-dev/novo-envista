...
function Header() {
  return <header className={styles.header}><div className={styles.headerInner}>
    ...
    <nav className={styles.desktopNav} aria-label="Navegação principal"><Link href="/schools">Para escolas</Link><Link href="/login">Entrar</Link><Link className={styles.headerCta} href="/register">Criar conta</Link></nav>
    <details className={styles.mobileNav}><summary aria-label="Abrir navegação"><Menu size={20} aria-hidden="true" /></summary><nav aria-label="Navegação principal no celular"><Link href="/schools">Para escolas</Link><Link href="/login">Entrar</Link><Link className={styles.headerCta} href="/register">Criar conta</Link></nav></details>
  </div></header>;
}
  ...
  return <main className={styles.page}>
    ...
    <div id="conteudo">
      <section className={styles.hero} style={{ gridTemplateColumns: "minmax(0,1.15fr) minmax(360px,.85fr)" }}>
        <div className={styles.heroCopy}><span className={styles.kicker}>ENVISTA PARA ESCOLAS</span><h1>Projetos dos alunos não precisam terminar na apresentação.</h1><p>O Envista cria continuidade entre aprendizagem prática, formação de equipes, desenvolvimento de projetos e descoberta de oportunidades.</p><div className={styles.heroActions}><Link className={styles.primary} href="/register">Experimentar o Envista <ArrowRight size={17} aria-hidden="true" /></Link><Link className={styles.secondary} href="/">Conhecer a plataforma</Link></div></div>
