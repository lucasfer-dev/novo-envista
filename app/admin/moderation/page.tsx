...
function targetLabel(type:string,row:any){
 if(!row)return "Conteúdo indisponível.";
 if(type==="profile")return `${row.display_name || row.username || "Perfil"}${row.username?` (@${row.username})`:""}`;
 if(type==="post")return row.body || "Publicação";
 if(type==="project")return row.title || "Projeto";
 if(type==="team")return row.name || "Equipe";
 return "Conteúdo";
}
...
export default async function AdminModeration({searchParams}:{searchParams:Promise<Record<string,string|string[]|undefined>>}){
 ...
 const [{data:reports},{data:contentReports}]=await Promise.all([
  supabase.from("message_reports").select("id,reporter_id,message_id,reason,details,status,admin_note,created_at,resolved_at").order("created_at",{ascending:false}).limit(100),
  supabase.from("content_reports").select("id,reporter_id,target_type,target_id,reason,details,status,admin_note,created_at,resolved_at").order("created_at",{ascending:false}).limit(100),
 ]);
 ...
 const [targetProfilesResult,targetPostsResult,targetProjectsResult,targetTeamsResult]=await Promise.all([
  ...
  teamTargetIds.length?supabase.from("teams").select("id,name,slug,owner_user_id").in("id",teamTargetIds):empty,
 ]);

 const profileIds=Array.from(new Set([
  ...(reports??[]).map((r:any)=>r.reporter_id),
  ...(contentReports??[]).map((r:any)=>r.reporter_id),
 ...
 const targetMaps={
  profile:new Map((targetProfilesResult.data??[]).map((row:any)=>[row.id,row])),
  post:new Map((targetPostsResult.data??[]).map((row:any)=>[row.id,row])),
  project:new Map((targetProjectsResult.data??[]).map((row:any)=>[row.id,row])),
  team:new Map((targetTeamsResult.data??[]).map((row:any)=>[row.id,row])),
 ...
 return <AdminShell profile={profile} title="Moderação">
  ...
  <div className={styles.stack}>{(contentReports??[]).length===0?<div className={styles.empty}>Nenhuma denúncia de conteúdo registrada.</div>:(contentReports??[]).map((report:any)=>{
   const reporter=profileMap.get(report.reporter_id);const target=targetMaps[report.target_type]?.get(report.target_id);
