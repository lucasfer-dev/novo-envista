...
export const metadata: Metadata = { title: "Entrar", description: "Entre no Envista e continue seus projetos, equipes e oportunidades." };
...
export default async function LoginPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  if (!claimsError && userId) {
    ...
    if (!profileResult.error && !complianceResult.error && !completionResult.error) {
      ...
      if (complianceResult.data?.age_band === "child" && !complianceResult.data.guardian_consent_verified_at) redirect("/guardian-required");
      ...
      redirect(pathAllowedForRole(destination, role) ? destination : fallback);
    }
  }
  ...
  return (
    <AuthShell title="Entrar no Envista" description="Continue de onde parou e acompanhe seus projetos, equipes e oportunidades.">
      ...
      <p className={styles.sessionHint}>Sua sessão é mantida neste navegador enquanto continuar válida.</p>
      <div className={styles.authFooter}><span className={styles.footerPrompt}>Ainda não faz parte?</span><Link className={styles.secondary} href="/register">Criar conta</Link></div>
    </AuthShell>
  );
}
