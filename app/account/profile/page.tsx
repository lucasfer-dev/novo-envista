...
export default async function AccountProfilePage({
  ...
  const [{ data: profile }, { data: compliance }, { data: completion }] = await Promise.all([
    supabase
      .from("profiles")
      .select("username,display_name,role,bio,public_city,public_state,public_school,organization,organization_type,profile_visibility,allow_messages")
      .eq("id", userId)
      .single(),
    supabase
    ...
    <AuthShell wide title="Meu perfil" description="Esses dados já são persistidos no Supabase. Você controla o que aparece para outras pessoas.">
      ...
      {error ? (
        <div className={styles.error}>
          {error === "username" ? "Esse nome de usuário já está em uso." : "Não foi possível salvar as alterações."}
        </div>
      ) : null}
      <form action={profileUpdateAction} className={styles.form}>
        ...
        <label>
          ...
          <select name="profile_visibility" defaultValue={profile.profile_visibility} disabled={isChild}>
            <option value="private">Privado</option>
            {!isChild ? <option value="platform">Visível para usuários autenticados</option> : null}
          </select>
        </label>
        {isChild ? <input type="hidden" name="profile_visibility" value="private" /> : null}

        <label className={styles.check}>
          <input type="checkbox" name="allow_messages" defaultChecked={profile.allow_messages} disabled={isChild} />
          <span>{isChild ? "Mensagens permanecem desativadas para esta faixa etária." : "Permitir que outros usuários autenticados iniciem uma conversa comigo."}</span>
        </label>

        <div className={styles.actions}>
          <button className={styles.primary} type="submit">Salvar perfil</button>
