...
export default async function FeedbackPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  return (
    <AccountProductShell
      ...
      description="Encontrou algo estranho ou teve uma ideia? Envie direto para a equipe do Envista."
    >
      ...
      {(tickets ?? []).length ? (
        <>
          ...
          <h2 style={{ fontSize: 18 }}>Minhas solicitações</h2>
          <div className={styles.legal}>
            <ul>
              {(tickets ?? []).map((ticket: any) => (
                <li key={ticket.id}>
                  <strong>{labels[ticket.category] || ticket.category}</strong> · {ticket.status} · {new Date(ticket.created_at).toLocaleDateString("pt-BR")}<br />
                  <span>{ticket.message.slice(0, 180)}{ticket.message.length > 180 ? "…" : ""}</span>
                </li>
              ))}
            </ul>
          </div>
